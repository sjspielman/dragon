...
{ 
  ...
  { ## Has at least one element
    rruff_separated %>%
      dplyr::group_by(mineral_name) %>%
      dplyr::mutate(has_element = ifelse( element %in% elements_of_interest, TRUE, FALSE)) %>% 
      dplyr::filter(has_element == TRUE) %>%
      dplyr::select(mineral_name) %>%
...
construct_network   <- function(network_information, elements_by_redox)
{
  ...
  V(element_network)$type <- igraph::bipartite_mapping(element_network)$type 
