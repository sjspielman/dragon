...
To perform a linear regression, specify a *predictor* (aka "explanatory" or "independent") variable and a *response* (aka "dependent") variable. The modeling results will be reported on the left side of the left side of the screen. A visualization of your variables will appear on the right side of the screen, which can be styled according to various options that appear below the visualization. You can also export this image using the "Download Plot" button. Importantly, if you select "Community cluster" as the predictor variable, a Tukey test will also be performed to perform pairwise comparisons of all clusters considered. 
...
+ Any network with fewer than three minerals cannot be analyzed. `dragon` will issue a warning prompt if there are too few data points for statistical analysis.
...
+ `dragon` will _not_ transform the data in any way. 
+ For analyses which consider Community Cluster as the predictor variable, only those clusters that contain at least 3 minerals will be analyzed. *Please interpret these results with caution.*
