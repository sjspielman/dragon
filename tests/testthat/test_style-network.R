...
test_that("fct_style_network::style_nodes() node dynamic colors", {
  ...
  style_options_here <- style_options_test
  style_options_here[["element_color_by"]] <- "num_localities" 
  style_options_here[["mineral_color_by"]] <- "num_localities"
  
  styled_test <- style_nodes(full_nodes, style_options_here)
  styled_nodes <- styled_test$styled_nodes
  
  # Should have bipartitde legend
  expect_true(styled_test[["both_legend"]] == FALSE)
  expect_true(typeof(styled_test[["element_legend"]]) == "list")
  expect_true(typeof(styled_test[["mineral_legend"]]) == "list")
...
test_that("fct_style_network::style_nodes() node dynamic sizes", {
  ...
  expect_true(sum(is.na(styled_nodes$size[styled_nodes$group == "element"]))  != length(styled_nodes$size[styled_nodes$group == "element"]))
  ...
  expect_true(sum(is.na(styled_nodes$size[styled_nodes$group == "mineral"]))  != length(styled_nodes$size[styled_nodes$group == "mineral"]))
