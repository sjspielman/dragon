test_that("fct_build-legend::build_legend() returns a legend", {
  ...
  fake_plot <- ggplot2::ggplot(iris) + ggplot2::aes(x = Sepal.Length) + ggplot2::geom_histogram()
  edge_styler_na <- list("edge_legend" = NA)
  edge_styler_with <- list("edge_legend" = fake_plot)
  node_styler_both <- list("both_legend" = fake_plot)
  node_styler_separate <- list("both_legend" = NA, "mineral_legend" = fake_plot, "element_legend" = fake_plot)
